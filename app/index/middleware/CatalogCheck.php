...
class CatalogCheck
{
    ...
    {
        CatalogService::init($request);
        // 头部底部菜单
        $catalogHeader = [];
        $catalogFooter = [];
        foreach (CatalogService::$catalogList as $val) {
        ...
        CatalogService::$catalogHeader = $header->leaf(0);
        CatalogService::$catalogFooter = $footer->leaf(0);
        // 当前分类
        $catalog = [];
        ...
        // 站内链接
        if (empty($catalog)) {
            foreach (CatalogService::$catalogList as $key => $val) {
                if ($val['links_type'] == 1) {
...
//                'group_id'        => [],
        ...
        // 分类等级
        foreach (CatalogService::$crumbs as $key => $val) {
            $level = 'level' . ($key + 1);

    ...
    <el-scrollbar v-loading="baTable.form.loading" class="ba-table-form-scrollbar">
      ...
      >
        <el-form
          ...
          :rules="rules"
        >
          <FormItem :label="t('cms.slide.data.slide_id')" type="remoteSelect" v-model="baTable.form.items!.slide_id"
                    prop="slide_id"
                    :input-attr="{ pk: 'ba_cms_slide.id', field: 'name', 'remote-url': '/admin/cms.Slide/index' }"
                    :placeholder="t('Please select field', { field: t('cms.slide.data.slide_id') })"/>
          <FormItem :label="t('cms.slide.data.title')" type="string" v-model="baTable.form.items!.title" prop="title"
                    :placeholder="t('Please input field', { field: t('cms.slide.data.title') })"/>
          ...
          <el-form-item v-for="(item, index) in slide.extends" :label="item.label" :key="index">
            <div class="w100">
              ...
              <CustomArray v-else-if="item.type!.type == 'customArray'"
                           v-model="baTable.form.items!.extends[item.field]"/>
              <BaInput v-else @pointerdown.stop :type="item.type!.type"
                       v-model="baTable.form.items!.extends[item.field]"/>
            </div>
          </el-form-item>
          ...
          <FormItem :label="t('cms.slide.data.remark')" type="textarea" v-model="baTable.form.items!.remark"
                    prop="remark" :placeholder="t('Please input field', { field: t('cms.slide.data.remark') })"/>
          <FormItem :label="t('cms.slide.data.width')" type="number" prop="width" :input-attr="{ step: 1 }"
                    v-model.number="baTable.form.items!.width"
                    :placeholder="t('Please input field', { field: t('cms.slide.data.width') })"/>
          <FormItem :label="t('cms.slide.data.height')" type="number" prop="height" :input-attr="{ step: 1 }"
                    v-model.number="baTable.form.items!.height"
                    :placeholder="t('Please input field', { field: t('cms.slide.data.height') })"/>
          <FormItem :label="t('cms.slide.data.status')" type="radio" v-model="baTable.form.items!.status" prop="status"
                    :data="{ content: { '0': t('cms.slide.data.status 0'), '1': t('cms.slide.data.status 1') } }"
                    :placeholder="t('Please select field', { field: t('cms.slide.data.status') })"/>
          <el-form-item :label="t('cms.slide.data.group')" prop="group">
            <el-select v-model="baTable.form.items!.group" clearable
                       :placeholder="t('Please input field', { field: t('cms.slide.data.group') })" class="w100"
